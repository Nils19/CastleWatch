...
try:
    ...
    print(
        "Warning: OpenCV not installed. To use motion detection, make sure you've properly configured OpenCV.",
    )
...
class Turret:
    ...
    def calibrate(self):
        ...
        """
        print(
            'Please calibrate the tilt of the gun so that it is level. Commands: (w) moves up, '
            '(s) moves down. Press (enter) to finish.\n',
        )
        self.__calibrate_y_axis()

        print(
            'Please calibrate the yaw of the gun so that it aligns with the camera. Commands: (a) moves left, '
    ...
    def __move_axis(self, contour, frame):
        ...
        print('x: {}, y: {}'.format(str(target_steps_x), str(target_steps_y)))
        print(
        ...
        if not self.friendly_mode:
            if (
                ...
                and abs(target_steps_x - self.current_x_steps) <= 2
            ):
    ...
    def interactive(self):
        ...
        print(
            'Commands: Pivot with (a) and (d). Tilt with (w) and (s). Exit with (q)\n',
        )
        with raw_mode(sys.stdin):
            try:
                while True:
    ...
    def move_forward(motor, steps):
        ...
        """
        motor.step(
            steps, Adafruit_MotorHAT.FORWARD, Adafruit_MotorHAT.INTERLEAVE,
        )
    ...
    def move_backward(motor, steps):
        ...
        """
        motor.step(
            steps, Adafruit_MotorHAT.BACKWARD, Adafruit_MotorHAT.INTERLEAVE,
        )
